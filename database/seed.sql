...
WITH t AS (
  INSERT INTO kaltim.tests (id, name, description, type) VALUES
    (gen_random_uuid(), 'Digital Marketing Dasar', 'Test kemampuan dasar digital marketing untuk UMKM', 'test')
  RETURNING id
),
pre AS (
  INSERT INTO kaltim.test_phases (id, test_id, phase, label, sort_order)
...
INSERT INTO kaltim.test_questions (id, phase_id, question_text, question_type, options, correct_answer, points, sort_order)
...
INSERT INTO kaltim.test_questions (id, phase_id, question_text, question_type, points, sort_order)
...
INSERT INTO kaltim.test_questions (id, phase_id, question_text, question_type, options, points, sort_order)
VALUES
...
INSERT INTO kaltim.event_tests (event_id, phase_id, open_time, is_open)
SELECT
  e.id AS event_id,
  tp.id AS phase_id,
  CASE
  ...
  END AS open_time,
  true AS is_open
FROM kaltim.events e
CROSS JOIN kaltim.test_phases tp
JOIN kaltim.tests t ON t.id = tp.test_id
...
INSERT INTO kaltim.event_invitations (event_id, umkm_id, status, sent_at)
SELECT e.id, u.id, 'attended', now() - interval '2 days'
FROM kaltim.events e, kaltim.umkm u
WHERE e.title = 'Pelatihan Digital Marketing UMKM'
...
INSERT INTO kaltim.test_answers (question_id, event_id, umkm_id, answer_text, score, submitted_at)
SELECT q.id, e.id, u.id,
  ...
  END,
  now() - interval '1 day'
FROM kaltim.test_questions q
CROSS JOIN kaltim.events e
CROSS JOIN kaltim.umkm u
WHERE e.title = 'Pelatihan Digital Marketing UMKM'
  AND u.business_name = 'Andi Bakery'
  AND q.phase_id = (SELECT tp.id FROM kaltim.test_phases tp JOIN kaltim.tests t ON t.id = tp.test_id WHERE tp.label = 'Pre-Test' AND t.name = 'Digital Marketing Dasar' LIMIT 1);
...
WHERE e.title = 'Pelatihan Digital Marketing UMKM'
  AND u.business_name = 'Andi Bakery'
  AND q.phase_id = (SELECT tp.id FROM kaltim.test_phases tp JOIN kaltim.tests t ON t.id = tp.test_id WHERE tp.label = 'Post-Test' AND t.name = 'Digital Marketing Dasar' LIMIT 1);
...
INSERT INTO kaltim.test_answers (question_id, event_id, umkm_id, answer_text, submitted_at)
...
FROM (
  ...
  FROM kaltim.test_questions q
  JOIN kaltim.test_phases tp ON tp.id = q.phase_id
  JOIN kaltim.tests t ON t.id = tp.test_id
...
-- VERIFICATION
-- ═══════════════════════════════════════════════════════════════
SELECT 'tests' AS tbl, COUNT(*)::text AS cnt FROM kaltim.tests
UNION ALL SELECT 'test_phases', COUNT(*)::text FROM kaltim.test_phases
UNION ALL SELECT 'test_questions', COUNT(*)::text FROM kaltim.test_questions
UNION ALL SELECT 'event_tests', COUNT(*)::text FROM kaltim.event_tests
UNION ALL SELECT 'event_invitations', COUNT(*)::text FROM kaltim.event_invitations
UNION ALL SELECT 'test_answers', COUNT(*)::text FROM kaltim.test_answers
ORDER BY tbl;

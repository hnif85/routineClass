...
export default function DashboardPage() {
  ...
  if (d.loading) {
    return <div style={{ padding: 48, textAlign: 'center', color: '#73837A', fontSize: 14 }}>Memuat dashboard...</div>;
  }
    ...
    <div style={{ animation: 'fade-in-up 0.5s ease-out both' }}>
      ...
      <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'stretch' : 'flex-start', gap: 18, flexWrap: 'wrap', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: '#73837A', fontSize: 13.5, marginTop: 6, maxWidth: isMobile ? '100%' : 520 }}>
            Pantau perkembangan {(totalUmkm || 0).toLocaleString("id-ID")} UMKM binaan PKT di Bontang — dari legalitas hingga event pemberdayaan.
          </p>
        </div>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: kpiCols, gap: isMobile ? 12 : 18, marginBottom: 24 }}>
        ...
        <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 18, padding: isMobile ? 16 : 22, boxShadow: 'var(--shadow)', position: 'relative', overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <div style={{ width: 34, height: 34, borderRadius: 10, display: 'grid', placeItems: 'center', background: '#DFF5E8', color: '#1F9D5A' }}>
            ...
            <span style={{ fontSize: 10.5, fontWeight: 700, padding: '3px 8px', borderRadius: 999, background: '#DFF5E8', color: '#1F9D5A' }}>Aktif</span>
          </div>
          <div style={{ fontSize: 12, fontWeight: 600, color: '#73837A' }}>Total UMKM Binaan</div>
          ...
          <div style={{ fontSize: 12, color: '#3C4A42', fontWeight: 500 }}>
            Tersebar di <b style={{ color: '#152019', fontWeight: 700 }}>{Object.keys(cityCounts).length} kecamatan</b> Bontang
          </div>
        </div>
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: 12, fontWeight: 600, color: '#73837A' }}>Kepemilikan NIB</div>
            <div style={{ fontSize: 12, color: '#3C4A42', fontWeight: 500, marginTop: 4 }}>
              <b className="num" style={{ color: '#152019', fontWeight: 700 }}>{withNib || 0}</b> dari {totalUmkm || 0} UMKM sudah ber-NIB
            </div>
            <div style={{ marginTop: 10, height: 5, borderRadius: 999, background: '#EEF1EA', overflow: 'hidden' }}>
              <div style={{ height: '100%', borderRadius: 999, background: 'linear-gradient(90deg, #2FB36B, #1F9D5A)', width: `${nibPercent}%`, transition: 'width 1.1s cubic-bezier(.22,1,.36,1)' }} />
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10.5, color: '#73837A', marginTop: 6, fontWeight: 600 }}>
          ...
          {!isMobile && (
            <div style={{ width: 100, height: 100, position: 'relative', flex: '0 0 auto' }}>
              <svg width="100" height="100" viewBox="0 0 108 108" style={{ transform: 'rotate(-90deg)' }}>
                ...
                <defs><linearGradient id="gleaf" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stopColor="#3BC177" /><stop offset="1" stopColor="#1F9D5A" /></linearGradient></defs>
              </svg>
              <div style={{ position: 'absolute', inset: 0, display: 'grid', placeContent: 'center', textAlign: 'center' }}>
                <div className="num" style={{ fontSize: 24, fontWeight: 800, lineHeight: 1, letterSpacing: '-0.02em' }}>{nibPercent}%</div>
                <div style={{ fontSize: 9, fontWeight: 700, color: '#73837A', marginTop: 2 }}>BER-NIB</div>
              </div>
            </div>
          )}
          {isMobile && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ flex: 1, height: 40, borderRadius: 10, background: '#EEF1EA', position: 'relative', overflow: 'hidden' }}>
                <div style={{ height: '100%', borderRadius: 10, background: 'linear-gradient(90deg, #2FB36B, #1F9D5A)', width: `${nibPercent}%`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <span style={{ fontSize: 13, fontWeight: 800, color: '#fff' }}>{nibPercent}%</span>
                </div>
              </div>
              <span style={{ fontSize: 11, fontWeight: 700, color: '#73837A' }}>BER-NIB</span>
            </div>
          )}
        </div>
        ...
        <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 18, padding: isMobile ? 16 : 22, boxShadow: 'var(--shadow)', position: 'relative', overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            ...
            <span style={{ fontSize: 10.5, fontWeight: 700, padding: '3px 8px', borderRadius: 999, background: '#F0F2EC', color: '#73837A' }}>30 hari</span>
          </div>
          <div style={{ fontSize: 12, fontWeight: 600, color: '#73837A' }}>Event Mendatang</div>
          <div className="num" style={{ fontSize: kpiFontSize, fontWeight: 800, lineHeight: 1, margin: '3px 0 6px', letterSpacing: '-0.025em' }}>
          ...
          {upcomingEvents && upcomingEvents.length > 0 && (
            <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap' }}>
              {upcomingEvents.slice(0, 3).map((ev: any) => {
                ...
                return (
                  <div key={ev.id} style={{ fontSize: 11, fontWeight: 700, color: '#3C4A42', background: '#F4F6F0', border: '1px solid var(--border-2)', borderRadius: 8, padding: '4px 8px', display: 'flex', gap: 5, alignItems: 'center' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: mainGrid, gap: 18, alignItems: 'start' }}>
        ...
        <section>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: panelCols, gap: isMobile ? 12 : 14 }}>
            <CtxBox title="Per Kategori" count={`${topCategories.length} jenis`}>
              ...
              {topCategories.length === 0 && <div style={{ fontSize: 13, color: '#73837A', textAlign: 'center', padding: 20 }}>Belum ada data</div>}
            </CtxBox>
            <CtxBox title="Per Kecamatan" count={`${topCities.length} wilayah`}>
              ...
              {topCities.length > 0 && (
                <div style={{ marginTop: 14, paddingTop: 12, borderTop: '1px dashed var(--border)', fontSize: 11.5, color: '#73837A', fontWeight: 600 }}>
                  Konsentrasi tertinggi di <b style={{ color: '#152019' }}>{topCities[0][0]}</b>
                </div>
              )}
            </CtxBox>
            <CtxBox title="Per Omzet / Bulan" count="Rp">
                ...
                return (
                  <div key={r} style={{ marginBottom: 12 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 5 }}>
                      <span style={{ fontSize: 12.5, fontWeight: 600, color: isZero ? '#73837A' : '#152019' }}>{revLabels[r] || r}</span>
                      <span className="num" style={{ fontSize: 13, fontWeight: 700, color: isZero ? '#73837A' : '#152019' }}>{cnt}</span>
                    </div>
              ...
              <div style={{ marginTop: 14, paddingTop: 12, borderTop: '1px dashed var(--border)', fontSize: 11.5, color: '#73837A', fontWeight: 600 }}>
                Mayoritas di segmen <b style={{ color: '#152019' }}>Rp 5–15 jt</b>
              </div>
            </CtxBox>
          </div>
        </section>
        ...
        <section>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {(!upcomingEvents || upcomingEvents.length === 0) && (
              <div style={{ border: '1.5px dashed var(--border)', borderRadius: 18, padding: 24, textAlign: 'center', color: '#73837A', fontSize: 13, fontWeight: 600 }}>
            ...
            {upcomingEvents?.map((ev: any, i: number) => {
              ...
              return (
                <Link key={ev.id} href={`/events/${ev.id}`} className="card-hover" style={{
                  ...
                  <span style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 4, background: isAmber ? '#E2A33A' : '#2FB36B' }} />
                  <div style={{ flex: '0 0 auto', width: 48, textAlign: 'center', borderRadius: 10, padding: isMobile ? '6px 0' : '10px 0', background: isAmber ? '#FBEFD6' : '#DFF5E8', color: isAmber ? '#B57A1E' : '#1F9D5A' }}>
                  ...
                  <div style={{ minWidth: 0, flex: 1 }}>
                    <div style={{ fontSize: isMobile ? 13.5 : 14.5, fontWeight: 700, lineHeight: 1.25, color: '#152019' }}>{ev.title}</div>
                    <div style={{ fontSize: isMobile ? 11.5 : 12.5, color: '#73837A', marginTop: 4, display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap' }}>
                      ...
                      {ev.location || "Online"}
                      <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', padding: '2px 6px', borderRadius: 5, background: '#F0F2EC', color: '#3C4A42' }}>{ev.type || "offline"}</span>
                    </div>
                  </div>
                </Link>
              );
            ...
            <Link href="/events/new" className="hover:border-[var(--leaf)] hover:text-[var(--leaf-strong)]" style={{
              ...
              color: '#73837A', fontSize: 13, fontWeight: 600, cursor: 'pointer', transition: 'all 0.18s',
...
function SectionHeader({ title, count }: { title: string; count?: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
      ...
      {count && <span style={{ fontSize: 11.5, fontWeight: 700, color: '#73837A', background: '#fff', border: '1px solid var(--border)', padding: '3px 9px', borderRadius: 999 }}>{count}</span>}
    </div>
  );
}
...
function RowBar({ label, value, maxVal, colorClass }: { label: string; value: number; maxVal: number; colorClass?: string }) {
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 5 }}>
        <span style={{ fontSize: 12.5, fontWeight: 600, color: '#152019' }}>{label}</span>
        <span className="num" style={{ fontSize: 13, fontWeight: 700, color: '#152019' }}>{value}</span>
      </div>
      <div style={{ height: 6, borderRadius: 999, background: '#F0F2EC', overflow: 'hidden' }}>
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 18, padding: 16, boxShadow: 'var(--shadow)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
        <span style={{ fontSize: 10.5, fontWeight: 800, letterSpacing: '0.13em', textTransform: 'uppercase', color: '#3C4A42' }}>{title}</span>
        <span style={{ fontSize: 10.5, fontWeight: 700, color: '#73837A' }}>{count}</span>
      </div>
      {children}
    </div>
  );
